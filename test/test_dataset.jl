...
if true # create local scope
	...
	test_vec_approx_equal( dset[indA, 0:2], [0.0, 0.1, 0.2] )
	test_vec_approx_equal( dset[indA], [0.0, 0.1, 0.2, 0.3] )
	...
	for (i,v) in enumerate(mat)
		@test isapprox(get_float(dset2, indA, i-1), v)
	end
	...
	for (i,v) in enumerate(mat)
		@test isapprox(get_float(dset3, indA, i-1), v)
	end
	...
	for (i,v) in enumerate(mat)
		@test isapprox(get_float(dset4, indA, i-1), v)
	end
	rm("out.dat")
